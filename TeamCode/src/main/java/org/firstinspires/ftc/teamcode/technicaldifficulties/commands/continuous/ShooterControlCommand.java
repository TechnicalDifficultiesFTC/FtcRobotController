...
public class ShooterControlCommand implements Command {
    ...
    public void periodic() {
        ...
        // Shooter Speed
        if(gunnerGamepad.dpad_up) shooter.incrementShooterWheelPower(0.05);
        else if(gunnerGamepad.dpad_down) shooter.incrementShooterWheelPower(-0.05);

        // Shooter Ramp (REDO TO MOVE DEGREES)
        if(gunnerGamepad.y) shooter.incrementShooterRampPosition(0.005);
        else if(gunnerGamepad.a) shooter.incrementShooterRampPosition(-0.005);
    }
